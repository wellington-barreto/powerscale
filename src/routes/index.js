...
A.get('/financial/entries',async(req,res,next)=>{try{let q=admin.from('financial_entries').select('*,category:financial_categories(*)').eq('workspace_id',req.workspaceId);if(req.query.year)q=q.gte('entry_date',`${req.query.year}-01-01`).lte('entry_date',`${req.query.year}-12-31`);const data=await one(q.order('entry_date',{ascending:false}));res.json({data:(data||[]).map(entryOut)})}catch(e){next(e)}});
