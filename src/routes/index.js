...
router.post('/google-ads/import/:uuid',async(req,res,next)=>{try{
  ...
  for(const [externalId,a] of accounts){
    const existing=await one(admin.from('google_ads_accounts').select('id,name,currency_code,timezone,payload').eq('workspace_id',member.workspace_id).eq('external_id',externalId).maybeSingle());
    const incomingCurrency=String(a.currency||a.currency_code||'').trim().toUpperCase()||null;
    const incomingTimezone=String(a.timezone||'').trim()||null;
    const payload={...(existing?.payload||{}),...(a||{})};
    if(!payload.currency && (incomingCurrency||existing?.currency_code))payload.currency=incomingCurrency||existing.currency_code;
    if(!payload.timezone && (incomingTimezone||existing?.timezone))payload.timezone=incomingTimezone||existing.timezone;
    const {data,error}=await admin.from('google_ads_accounts').upsert({workspace_id:member.workspace_id,external_id:externalId,name:a.name||existing?.name||null,currency_code:incomingCurrency||existing?.currency_code||null,timezone:incomingTimezone||existing?.timezone||null,source:'appscript',status:'active',sync_enabled:true,payload},{onConflict:'workspace_id,external_id'}).select('id,external_id').single();
    if(error)throw error;accountDb.set(externalId,data.id);
  }
...
A.get('/platforms',async(req,res,next)=>{try{const rows=await one(admin.from('platforms').select('*').eq('workspace_id',req.workspaceId).order('name'));res.json((rows||[]).map(p=>({...p,logo:p.logo||p.logo_url||null}))) }catch(e){next(e)}});
...
function normalizedCurrency(account){
  const raw=account?.currency_code||account?.payload?.currency||account?.payload?.currency_code||null;
...
A.get('/trackers',async(req,res,next)=>{try{
  ...
  const accounts=accountIds.length?await one(admin.from('google_ads_accounts').select('id,currency_code,payload').in('id',accountIds)):[];
  const accountCurrency=new Map(accounts.map(a=>[a.id,normalizedCurrency(a)]));
...
A.post('/google-ads/accounts',async(req,res,next)=>{try{
  const {start_date,end_date}=req.body||{};
  const accounts=await one(admin.from('google_ads_accounts').select('*').eq('workspace_id',req.workspaceId).eq('sync_enabled',true));
  for(const a of accounts)a.currency_code=normalizedCurrency(a);
  const accountIds=accounts.map(a=>a.id); if(!accountIds.length)return res.json({data:[],start_date,end_date});
  const camps=await one(admin.from('google_ads_campaigns').select('*,tracker:trackers(*)').eq('workspace_id',req.workspaceId).in('account_id',accountIds));
  const campaignIds=camps.map(c=>c.id); let metrics=[];
  if(campaignIds.length){let q=admin.from('google_ads_daily_metrics').select('campaign_id,impressions,clicks,cost,conversions,conversion_value,checkout_conversions,checkout_value,all_conversions,all_conversion_value').eq('workspace_id',req.workspaceId).in('campaign_id',campaignIds);if(start_date)q=q.gte('metric_date',start_date);if(end_date)q=q.lte('metric_date',end_date);metrics=await one(q)}
  const agg=new Map();for(const m of metrics){let a=agg.get(m.campaign_id);if(!a){a={};agg.set(m.campaign_id,a)}for(const k of ['impressions','clicks','cost','conversions','conversion_value','checkout_conversions','checkout_value','all_conversions','all_conversion_value'])a[k]=(a[k]||0)+Number(m[k]||0)}
  const byAccount=new Map(accounts.map(a=>[a.id,[]]));for(const c of camps){const p=c.payload||{},m=agg.get(c.id)||{};for(const k of ['bidding_strategy','target_cpa','target_roas','budget_daily','channel_type','channel_sub_type','max_cpc_limit','target_impression_share'])if(c[k]==null&&p[k]!=null)c[k]=p[k];for(const k of ['impressions','clicks','cost','conversions','conversion_value','checkout_conversions','checkout_value','all_conversions','all_conversion_value'])c['snapshots_sum_'+k]=Number(m[k]||0);c.snapshots_count=0;byAccount.get(c.account_id)?.push(c)}
  for(const a of accounts)a.campaigns=byAccount.get(a.id)||[];
  res.json({data:accounts,start_date,end_date})
}catch(e){next(e)}});
A.post('/google-ads/campaigns/:id/daily-metrics',async(req,res,next)=>{try{
  const campaignId=Number(req.params.id),campaign=await one(admin.from('google_ads_campaigns').select('id,account_id').eq('workspace_id',req.workspaceId).eq('id',campaignId).maybeSingle());
  if(!campaign)return res.status(404).json({message:'Campanha não encontrada'});
  const account=await one(admin.from('google_ads_accounts').select('currency_code,payload').eq('workspace_id',req.workspaceId).eq('id',campaign.account_id).maybeSingle()),currency_code=normalizedCurrency(account)||'BRL';
  let q=admin.from('google_ads_daily_metrics').select('*').eq('workspace_id',req.workspaceId).eq('campaign_id',campaignId);const b=req.body||{};if(b.start_date)q=q.gte('metric_date',String(b.start_date).slice(0,10));if(b.end_date)q=q.lte('metric_date',String(b.end_date).slice(0,10));const rows=await one(q.order('metric_date'));
  res.json({data:(rows||[]).map(x=>({...x,date:x.metric_date,currency_code})),currency_code})
}catch(e){next(e)}});

A.post('/google-ads/segments',async(req,res,next)=>{try{
  let campaignIds=null;
  if(req.body.tracker_id){const cs=await one(admin.from('google_ads_campaigns').select('id').eq('workspace_id',req.workspaceId).eq('tracker_id',req.body.tracker_id));campaignIds=cs.map(x=>x.id);if(!campaignIds.length){const out={};for(const t of req.body.segment_types||[])out[t]=[];return res.json({data:out})}}
  if(req.body.google_ads_campaign_id)campaignIds=[Number(req.body.google_ads_campaign_id)];
  let q=admin.from('google_ads_segments').select('*').eq('workspace_id',req.workspaceId);if(req.body.google_ads_campaign_id)q=q.eq('campaign_id',req.body.google_ads_campaign_id);else if(campaignIds)q=q.in('campaign_id',campaignIds);if(req.body.start_date)q=q.gte('segment_date',req.body.start_date);if(req.body.end_date)q=q.lte('segment_date',req.body.end_date);const rows=await one(q);
  const ids=[...new Set(rows.map(x=>Number(x.campaign_id)).filter(Boolean))];
  const campaigns=ids.length?await one(admin.from('google_ads_campaigns').select('id,account_id').eq('workspace_id',req.workspaceId).in('id',ids)):[];
  const accountIds=[...new Set(campaigns.map(x=>Number(x.account_id)).filter(Boolean))];
  const accounts=accountIds.length?await one(admin.from('google_ads_accounts').select('id,currency_code,payload').eq('workspace_id',req.workspaceId).in('id',accountIds)):[];
  const cmap=new Map(campaigns.map(x=>[Number(x.id),Number(x.account_id)])),amap=new Map(accounts.map(x=>[Number(x.id),normalizedCurrency(x)||'BRL']));
  const currencyFor=x=>amap.get(cmap.get(Number(x.campaign_id)))||'BRL';
  const additive=['impressions','clicks','cost','conversions','conversion_value','checkout_conversions','checkout_value','all_conversions','all_conversion_value','view_through_conversions','cross_device_conversions','interactions','invalid_clicks','engagements','active_view_impressions','gmail_forwards','gmail_saves','gmail_secondary_clicks'];
  const weighted=['average_cpc','average_cpm','ctr_percent','impression_share_percent','top_impression_share_percent','absolute_top_impression_share_percent','search_rank_lost_is_percent','search_rank_lost_top_is_percent','search_rank_lost_abs_top_is_percent','search_budget_lost_is_percent','search_budget_lost_top_is_percent','search_budget_lost_abs_top_is_percent','search_eligible_top_is_percent','search_eligible_abs_top_is_percent','search_exact_match_is_percent','search_click_share_percent','interaction_rate_percent','invalid_click_rate_percent','average_cost','engagement_rate','active_view_measurability','active_view_viewability'];
  const moneyFields=['cost','conversion_value','checkout_value','all_conversion_value'];
  const groups=new Map();
  for(const x of rows){
    const k=x.segment_type+'|'+(x.segment_key||''),cur=currencyFor(x),fresh=!groups.has(k),w=Number(x.impressions||0);
    if(fresh)groups.set(k,{...x,segment_data:x.dimension||x.payload?.[x.segment_type]||{},date_from:x.segment_date,date_to:x.segment_date,_w:w,currency_code:cur,money_by_currency:{}});
    const a=groups.get(k);
    if(!fresh){const ow=a._w||0;for(const c of additive)a[c]=Number(a[c]||0)+Number(x[c]||0);for(const c of weighted)if(x[c]!=null)a[c]=(a[c]==null||ow+w===0)?Number(x[c]):((Number(a[c])*ow+Number(x[c])*w)/(ow+w));a._w=ow+w;if(x.segment_date<a.date_from)a.date_from=x.segment_date;if(x.segment_date>a.date_to)a.date_to=x.segment_date;if(a.currency_code!==cur)a.currency_code='MULTI';}
    const b=a.money_by_currency[cur]||(a.money_by_currency[cur]={cost:0,conversion_value:0,checkout_value:0,all_conversion_value:0});for(const f of moneyFields)b[f]+=Number(x[f]||0);
  }
  const normalized=[...groups.values()].map(({_w,...x})=>{x.avg_cpc=x.average_cpc!=null?Number(x.average_cpc):((Number(x.clicks)||0)?Number(x.cost||0)/Number(x.clicks):0);x.avg_cpm=x.average_cpm!=null?Number(x.average_cpm):((Number(x.impressions)||0)?Number(x.cost||0)/Number(x.impressions)*1000:0);return x});
  const out={};for(const type of req.body.segment_types||[])out[type]=normalized.filter(x=>x.segment_type===type);res.json({data:out})
}catch(e){next(e)}});
A.post('/google-ads/metrics/funnel',async(req,res,next)=>{try{
  const b=req.body||{};let ids=[];if(b.google_ads_campaign_id)ids=[Number(b.google_ads_campaign_id)];else if(b.tracker_id){const cs=await one(admin.from('google_ads_campaigns').select('id').eq('workspace_id',req.workspaceId).eq('tracker_id',b.tracker_id));ids=cs.map(x=>x.id)}
  const campaigns=ids.length?await one(admin.from('google_ads_campaigns').select('id,account_id,name').eq('workspace_id',req.workspaceId).in('id',ids)):[];
  const accountIds=[...new Set(campaigns.map(x=>x.account_id).filter(Boolean))],accounts=accountIds.length?await one(admin.from('google_ads_accounts').select('id,currency_code,payload').eq('workspace_id',req.workspaceId).in('id',accountIds)):[];
  const cmap=new Map(campaigns.map(x=>[Number(x.id),x])),amap=new Map(accounts.map(x=>[Number(x.id),normalizedCurrency(x)||'BRL']));
  let rows=[];if(ids.length){let q=admin.from('google_ads_daily_metrics').select('*').eq('workspace_id',req.workspaceId).in('campaign_id',ids);const from=String(b.from||'').slice(0,10),to=String(b.to||'').slice(0,10);if(from)q=q.gte('metric_date',from);if(to)q=q.lte('metric_date',to);rows=await one(q.order('metric_date'))}
  const sum=k=>rows.reduce((z,x)=>z+Number(x[k]||0),0);const impressions=sum('impressions'),clicks=sum('clicks'),cost=sum('cost'),conversions=sum('conversions'),conversionValue=sum('conversion_value'),checkouts=sum('checkout_conversions');
  const money_by_currency={};const add=(cur,field,val)=>{const x=money_by_currency[cur]||(money_by_currency[cur]={cost:0,conversion_value:0,checkout_value:0,all_conversion_value:0});x[field]+=Number(val||0)};
  const timeline=rows.map(x=>{const camp=cmap.get(Number(x.campaign_id)),cur=amap.get(Number(camp?.account_id))||'BRL',money={[cur]:{cost:Number(x.cost||0),conversion_value:Number(x.conversion_value||0),checkout_value:Number(x.checkout_value||0),all_conversion_value:Number(x.all_conversion_value||0)}};add(cur,'cost',x.cost);add(cur,'conversion_value',x.conversion_value);add(cur,'checkout_value',x.checkout_value);add(cur,'all_conversion_value',x.all_conversion_value);return{date:x.metric_date,campaign_id:x.campaign_id,campaign_name:camp?.name||null,currency_code:cur,money_by_currency:money,impressions:Number(x.impressions||0),clicks:Number(x.clicks||0),cost:Number(x.cost||0),conversions:Number(x.conversions||0),conversion_value:Number(x.conversion_value||0),checkouts:Number(x.checkout_conversions||0)}});
  const currencies=Object.keys(money_by_currency),currency_code=currencies.length===1?currencies[0]:(currencies.length?'MULTI':null);
  res.json({data:{view_type:b.view_type||'google_ads',currency_code,money_by_currency,funnel:{impressions,clicks,page_views:0,passed:0,checkouts,purchases:conversions,conversions},cards:{investment:{value:cost},result:{value:conversions},conversion_value:{value:conversionValue},checkout_conversions:{value:checkouts},cost_per_result:{value:conversions?cost/conversions:0}},charts:{timeline_daily:timeline},steps:[]}})
}catch(e){next(e)}});
...
A.get('/google-ads/report-daily',async(req,res,next)=>{try{
  ...
  const outCampaign=campaign?{...campaign,currency_code:normalizedCurrency(campaign.account)||cp.currency_code||null,target_cpa:targetCpa,budget_daily:budget}:{};
...
// Finance compatibility layer: preserve the response/payload contract used by the original frontend.
A.get('/financial/categories',async(req,res,next)=>{try{const data=await one(admin.from('financial_categories').select('*').eq('workspace_id',req.workspaceId).order('id'));res.json({data})}catch(e){next(e)}});
A.post('/financial/categories',create('financial_categories')); A.put('/financial/categories/:id',update('financial_categories')); A.delete('/financial/categories/:id',remove('financial_categories'));
const entryOut=x=>({...x,group:x.type==='income'?'revenue':'expense',month:Number(String(x.entry_date||'').slice(5,7))||null,year:Number(String(x.entry_date||'').slice(0,4))||null,name:x.description||'',value:Number(x.amount||0),category:x.category||null});
A.get('/financial/entries',async(req,res,next)=>{try{let q=admin.from('financial_entries').select('*,category:financial_categories(*)').eq('workspace_id',req.workspaceId);if(req.query.year)q=q.gte('entry_date',`${req.query.year}-01-01`).lte('entry_date',`${req.query.year}-12-31`);const data=await one(q.order('entry_date',{ascending:false}));res.json({data:{entries:(data||[]).map(entryOut)}})}catch(e){next(e)}});
async function saveEntry(req,res,next,id=null){try{const b=req.body||{},year=Number(b.year||new Date().getFullYear()),month=Number(b.month||new Date().getMonth()+1);const entryDate=b.entry_date||`${year}-${String(month).padStart(2,'0')}-01`;const row={category_id:b.category_id||null,entry_date:entryDate,type:b.group==='revenue'?'income':'expense',description:b.name||b.description||null,amount:Number(b.value??b.amount??0),currency:String(b.currency||'BRL').toUpperCase()};let q=id?admin.from('financial_entries').update(row).eq('workspace_id',req.workspaceId).eq('id',id):admin.from('financial_entries').insert({workspace_id:req.workspaceId,...row});const data=await one(q.select('*,category:financial_categories(*)').single());res.json({data:entryOut(data)})}catch(e){next(e)}}
A.post('/financial/entries',(req,res,next)=>saveEntry(req,res,next)); A.put('/financial/entries/:id',(req,res,next)=>saveEntry(req,res,next,req.params.id)); A.delete('/financial/entries/:id',remove('financial_entries'));
const miningOut=x=>({...x,product_name:x.product_name||x.name||'',mining_status:x.mining_status||x.status||'em_teste',platform:x.platform?{...x.platform,logo:x.platform.logo||x.platform.logo_url||null}:null,...(x.payload||{})});
async function attachMiningPlatforms(rows,workspaceId){const list=Array.isArray(rows)?rows:[rows].filter(Boolean),ids=[...new Set(list.map(x=>Number(x.platform_id)).filter(Boolean))];let platforms=[];if(ids.length)platforms=await one(admin.from('platforms').select('*').eq('workspace_id',workspaceId).in('id',ids));const map=new Map((platforms||[]).map(p=>[Number(p.id),{...p,logo:p.logo||p.logo_url||null}]));return list.map(x=>miningOut({...x,platform:map.get(Number(x.platform_id))||null}));}
A.get('/financial/mining',async(req,res,next)=>{try{const rows=await one(admin.from('financial_mining').select('*').eq('workspace_id',req.workspaceId).order('id',{ascending:false}));const data=await attachMiningPlatforms(rows,req.workspaceId);res.json({data})}catch(e){next(e)}});
async function saveMining(req,res,next,id=null){try{const b=req.body||{};let existing=null;if(id)existing=await one(admin.from('financial_mining').select('*').eq('workspace_id',req.workspaceId).eq('id',id).single());const payload={...(existing?.payload||{})};for(const k of ['vol_fundo','vol_top_face','vol_top_similarweb','comissao_media','cpc_maior','cpc_medio','cpc_ideal','extra_col1_header','extra_col1_value','extra_col2_header','extra_col2_value','obs','mining_status'])if(k in b)payload[k]=b[k];const row={name:b.product_name??b.name??existing?.name,platform_id:b.platform_id===undefined?existing?.platform_id:b.platform_id,currency:b.currency||existing?.currency||'USD',status:b.mining_status||existing?.status||'em_teste',payload};if(!row.name)return res.status(422).json({message:'O nome do produto é obrigatório.'});let q=id?admin.from('financial_mining').update(row).eq('workspace_id',req.workspaceId).eq('id',id):admin.from('financial_mining').insert({workspace_id:req.workspaceId,...row});const saved=await one(q.select('*').single());const [data]=await attachMiningPlatforms([saved],req.workspaceId);res.json({data})}catch(e){next(e)}}
A.post('/financial/mining',(req,res,next)=>saveMining(req,res,next)); A.put('/financial/mining/:id',(req,res,next)=>saveMining(req,res,next,req.params.id)); A.delete('/financial/mining/:id',remove('financial_mining'));
...
A.get('/financial/company',async(req,res,next)=>{try{
  ...
  const [cats,entries,campaigns,accounts]=await Promise.all([
    one(admin.from('financial_categories').select('*').eq('workspace_id',req.workspaceId).order('id')),
    one(admin.from('financial_entries').select('*').eq('workspace_id',req.workspaceId).gte('entry_date',`${year}-01-01`).lte('entry_date',`${year}-12-31`)),
    one(admin.from('google_ads_campaigns').select('id,account_id').eq('workspace_id',req.workspaceId)),
    one(admin.from('google_ads_accounts').select('id,currency_code,payload').eq('workspace_id',req.workspaceId))
  ]);
  const ids=campaigns.map(c=>c.id);const metrics=ids.length?await one(admin.from('google_ads_daily_metrics').select('campaign_id,metric_date,cost,conversion_value').eq('workspace_id',req.workspaceId).in('campaign_id',ids).gte('metric_date',`${year}-01-01`).lte('metric_date',`${year}-12-31`)):[];
...
async function dashboardMetrics(req){
  ...
  const aids=[...new Set(campaigns.map(x=>x.account_id).filter(Boolean))]; const accounts=aids.length?await one(admin.from('google_ads_accounts').select('id,currency_code,name,payload').in('id',aids)):[]; for(const a of accounts)a.currency_code=normalizedCurrency(a); return{rows,campaigns,accounts};
}
A.post('/dashboard',async(req,res,next)=>{try{
  ...
  const byCurrency=new Map(),totalsByCurrency={};for(const r of rows){const cur=amap.get(cmap.get(r.campaign_id)?.account_id)?.currency_code||'UNKNOWN';const x=byCurrency.get(cur)||{currency:cur,amount:0};x.amount+=Number(r.conversion_value||0);byCurrency.set(cur,x);const b=totalsByCurrency[cur]||(totalsByCurrency[cur]={revenue:0,cost:0,profit:0});b.revenue+=Number(r.conversion_value||0);b.cost+=Number(r.cost||0);b.profit=b.revenue-b.cost;} const purchase=[...byCurrency.values()];
  ...
  const campaignAgg=new Map();for(const r of rows){const c=cmap.get(r.campaign_id);if(!c)continue;const cur=amap.get(c.account_id)?.currency_code||'BRL';const a=campaignAgg.get(c.id)||{id:c.id,name:c.name,currency_code:cur,cost:0,conversion_value:0,conversions:0};a.cost+=Number(r.cost||0);a.conversion_value+=Number(r.conversion_value||0);a.conversions+=Number(r.conversions||0);a.profit=a.conversion_value-a.cost;campaignAgg.set(c.id,a);}const top=[...campaignAgg.values()].sort((a,b)=>b.cost-a.cost);
  res.json({data:{cards:{purchase_by_currency:purchase,refund_by_currency:[],visitors:{value:visitors,trend:'flat',change_pct:0},checkouts:{value:checkouts,trend:'flat',change_pct:0},organic_sales:{value:0,trend:'flat',change_pct:0}},top_countries_sales:[],totals_by_currency:totalsByCurrency,totals:{revenue,investment:cost,cost,conversions,checkouts,impressions,clicks,profit,roi},revenue,investment:cost,cost,conversions,profit,roi,top_campaigns:top.slice(0,10),worst_campaigns:[...top].sort((a,b)=>(a.conversion_value-a.cost)-(b.conversion_value-b.cost)).slice(0,10)}});
}catch(e){next(e)}});
A.post('/dashboard/charts/sales',async(req,res,next)=>{try{const {rows,campaigns,accounts}=await dashboardMetrics(req);const amap=new Map(accounts.map(a=>[a.id,a])),cmap=new Map(campaigns.map(c=>[c.id,c])),m=new Map();for(const r of rows){const cur=amap.get(cmap.get(r.campaign_id)?.account_id)?.currency_code||'UNKNOWN',key=`${r.metric_date}|${cur}`,x=m.get(key)||{date:r.metric_date,currency:cur,amount:0,revenue:0,cost:0};x.amount+=Number(r.conversion_value||0);x.revenue+=Number(r.conversion_value||0);x.cost+=Number(r.cost||0);m.set(key,x);}res.json({charts:{sales_daily_by_currency:[...m.values()]},data:[...m.values()]})}catch(e){next(e)}});
// v13.4: câmbio é carregado exclusivamente no navegador para evitar rate-limit por IPs compartilhados da Vercel.
