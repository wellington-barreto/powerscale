...
A.post('/dashboard',async(req,res,next)=>{try{
  ...
  const byCurrency=new Map();for(const r of rows){const cur=amap.get(cmap.get(r.campaign_id)?.account_id)?.currency_code||'UNKNOWN';const x=byCurrency.get(cur)||{currency:cur,amount:0};x.amount+=Number(r.conversion_value||0);byCurrency.set(cur,x);} const purchase=[...byCurrency.values()];
